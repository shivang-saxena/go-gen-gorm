# go-gen-gorm [![GoDoc](https://godoc.org/github.com/shivang-saxena/go-gen-gorm?status.png)](http://godoc.org/github.com/shivang-saxena/go-gen-gorm) [![Go Report Card](https://goreportcard.com/badge/github.com/shivang-saxena/go-gen-gorm)](https://goreportcard.com/report/github.com/shivang-saxena/go-gen-gorm) [![Codacy Badge](https://api.codacy.com/project/badge/Grade/04f432950cbc4e8cb0ec4ef36f4e90bb)](https://www.codacy.com/app/shivang-saxena/go-gen-gorm?utm_source=github.com&amp;utm_medium=referral&amp;utm_content=shivang-saxena/go-gen-gorm&amp;utm_campaign=Badge_Grade) [![Maintainability](https://api.codeclimate.com/v1/badges/8a726af72f338e8998bd/maintainability)](https://codeclimate.com/github/shivang-saxena/go-gen-gorm/maintainability) [![Build Status](https://travis-ci.org/shivang-saxena/go-gen-gorm.svg?branch=master)](https://travis-ci.org/shivang-saxena/go-gen-gorm) [![Coverage Status](https://coveralls.io/repos/github/shivang-saxena/go-gen-gorm/badge.svg?branch=master)](https://coveralls.io/github/shivang-saxena/go-gen-gorm?branch=master)
...
# Installation
```bash
go get -u github.com/shivang-saxena/go-gen-gorm/cmd/go-gen-gorm
```

# Usage
## Define models
Imagine you have model `User` in your [`models.go`](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm1/gorm1.go#L16) file:
...
Now transform it by [adding comments for query set generation](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/gorm4.go#L15):
```go
...
And you will get file [`generated_models.go`](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/generated_gorm4.go) in the same directory (and package) as `models.go`.
...
See full autogenerated file [here](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/generated_gorm4.go).
...
```
Under the hood `Create` method [just calls `db.Create(&u)`](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/generated_gorm4.go#L38).
...
Goqueryset generates DB names for struct fields into [`UserDBSchema`](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/generated_gorm4.go#L414) variable.
In this example we used [`UserDBSchema.Rating`](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/generated_gorm4.go#L419).
...
# Golang version
Golang >= 1.8 is required. Tested on go 1.8, 1.9 versions by [Travis CI](https://travis-ci.org/shivang-saxena/go-gen-gorm)
...
## Why not any ORM?
I didn't see any ORM that properly handles code duplication. GORM is the best with `Scopes` support, but even it's far from ideal. E.g. we have GORM and [next typical code](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm1/gorm1.go#L16):
```go
type User struct {
	gorm.Model
	Rating      int
...
How to solve it? First idea is to [make reusable functions](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm2/gorm2.go#L27):
```go
...
We can use GORM [Scopes](http://jinzhu.me/gorm/crud.html#scopes) to improve [how it looks](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm3/gorm3.go#L64):
```go
...
Now compare it with [go-gen-gorm solution](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/gorm4.go#L30):
```go
...
1. It works only with PostgreSQL. go-gen-gorm supports mysql, postgresql, sqlite, mssql etc (all that gorm supports).
...
* QuerySet pattern allows to reuse queries by defining [custom methods](https://github.com/shivang-saxena/go-gen-gorm/blob/master/examples/comparison/gorm4/gorm4.go#L30) on it.
...
3. go-gen-gorm is fully compatible with GORM.
4. Code generation is used here not to speedup things, but to create nice interfaces.
5. The main purpose of go-gen-gorm isn't speed, but usage convenience.
